...
namespace BST_Winforms
{
    public partial class Form1 : Form
    {
        ...
        private string deleteTargetKey = null;
        private string promotedKey = null;
        private Stack<int> silinenlerStack = new Stack<int>();
        ...
        private async void btnDelete_Click(object sender, EventArgs e)
        {
            if (int.TryParse(txtInput.Text, out int value))
            {
                // Silmeden önce bağlı listeden kaldır
                ...
                // Animasyonu başlat ve ardından ekranı güncelle
        ...
        private async Task AnimateDelete(int value)
        {
            string key = value.ToString();

            // 🔶 Silinecek düğümü turuncu renkle vurgula
            deleteTargetKey = key;
            panelCanvas.Refresh();
            await Task.Delay(800);

            // 🟢 Successor varsa yeşil olarak göster
            var successor = bst.FindSuccessor(value);
            if (successor != null)
            {
                promotedKey = successor.Key.ToString();
                panelCanvas.Refresh();
                await Task.Delay(800);
            }
            ...
            // 🧱 Silinenler stack'e ekleniyor
            silinenlerStack.Push(value);
            GuncelleSilinenlerListesi();

            panelCanvas.Refresh();
        }

        private async void btnDeleteLast_Click(object sender, EventArgs e)
        {
            if (eklenenler.RemoveLast(out int lastValue))
            {
                await AnimateDelete(lastValue);               // 🔥 Silme animasyonu
                panelCanvas.Refresh();                        // Güncelleme
                GuncelleEklemeListesi();                      // Listeyi güncelle
            }
            else
            {
        ...
        private void GuncelleSilinenlerListesi()
        {
            listBoxSilinenler.Items.Clear();
            foreach (var val in silinenlerStack)
            {
                listBoxSilinenler.Items.Add(val);
            }
        }
        ...
        private void DrawNode(Graphics g, BSTNode node, int x, int y, int offset)
        {
            ...
            if (deleteTargetKey != null && node.Key.ToString() == deleteTargetKey)
                fillBrush = Brushes.OrangeRed;
            else if (promotedKey != null && node.Key.ToString() == promotedKey)
                fillBrush = Brushes.LimeGreen;
            else if (highlightedKey != null && node.Key.ToString() == highlightedKey)
